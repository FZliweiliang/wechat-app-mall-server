...
exports.cartList = (req,res) => {
    ...
    Cart.find({openid})
    .then(result=>{
        let cartList = result[0].goodsList
        let listId = []
        ...
        const filds = 'title img spec price num _id'

        Article.find({"_id":{$in:listId}},filds).then(result=>{
            result.map((v,k)=>{
                if(v._id == cartList[k].id){
                    v.num = cartList[k].num
                }
            })
            res.json({
                code: 200,
...
exports.editCart = (req,res) => {
    ...
    Cart.updateAsync({openid},{"goodsList":arr})
    .then(result=>{
        res.json({
            code: 200,
            message: '编辑完成',
